"""
This file aims to
train the DeepSurv model for predicting survival.
...
from DeepSurv.deepSurvModel import DeepSurvModel
...
def train():
    ...
    """(2) Prepare Network."""
    """Model."""
    model = DeepSurvModel().to(mc.device)
    ...
    for epoch in range(mc.epoch_start, mc.epoch_end):
        ...
        for i, patient_batch in enumerate(train_tqdm):
            ...
            """Predict."""
            predicted_survivals = model(text=text).to(mc.device)
