...
def modify_folder_names():
    ...
    :return: None
    """
    for root, dirs, files in os.walk(preConfig.input_path):
        for patient in dirs:
            try:
...
def get_min_max_layers_from_excel(excel_clinical, patient_id):
    ...
    for row in ws.iter_rows(min_row=2, max_row=ws.max_row):
        if row[preConfig.col_id].value == patient_id:
            min_layer = row[preConfig.col_min_layer].value
            max_layer = row[preConfig.col_max_layer].value
...
def write_layers_to_excel(excel_clinical, patient_id, min_layer, max_layer, total_layer):
    ...
    for row in ws.iter_rows(min_row=2, max_row=ws.max_row):
        if row[preConfig.col_id].value == patient_id:
            row[preConfig.col_min_layer].value = min_layer
            row[preConfig.col_max_layer].value = max_layer
            row[preConfig.col_total_layer].value = total_layer
            break
    wb.save(preConfig.excel_path)
...
def fill_excel_stage(excel_clinical):
    ...
    ws = wb[wb.sheetnames[0]]
    for row in ws.iter_rows(min_row=2, max_row=ws.max_row):
        T = row[preConfig.col_stage_T].value
        N = row[preConfig.col_stage_N].value
        if T is None or N is None:
            continue
        elif T == 'T1' and N == 'N0':
            row[preConfig.col_stage].value = 1
        elif (T == 'T2' and N == 'N0') or ((T == 'T1' or T == 'T2') and N == 'N1'):
            row[preConfig.col_stage].value = 2
        elif ((T == 'T1' or T == 'T2') and N == 'N2') or (T == 'T3' and (N == 'N0' or N == 'N1' or N == 'N2')):
            row[preConfig.col_stage].value = 3
        elif T == 'T4' and (N == 'N0' or N == 'N1' or N == 'N2'):
            row[preConfig.col_stage].value = 4
        elif N.startswith('N3'):
            row[preConfig.col_stage].value = 5
...
def fill_excel_missing_value(excel_clinical):
    ...
    stage_list = []
    for row in ws.iter_rows(min_row=2, max_row=ws.max_row):
        rt_days_list.append(row[preConfig.col_rt_days].value)
        BMI_list.append(row[preConfig.col_BMI].value)
        stage_list.append(row[preConfig.col_stage].value)
    ...
    for i, row in enumerate(ws.iter_rows(min_row=2, max_row=ws.max_row)):
        if row[preConfig.col_rt_days].value is None:
            row[preConfig.col_rt_days].value = data_mean_frame['rt_days'][i]
        if row[preConfig.col_BMI].value is None:
            row[preConfig.col_BMI].value = data_mean_frame['BMI'][i]
        if row[preConfig.col_stage].value is None:
            row[preConfig.col_stage].value = 3
    wb.save(excel_clinical)
